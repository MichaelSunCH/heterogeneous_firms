...
class HetBlock:
    ...
    def ss(self, backward_tol=1E-10, backward_maxit=5000, forward_tol=1E-10, forward_maxit=100_000, **kwargs):
    ...
    def ss_with_dist_iter(self, backward_tol=1E-8, backward_maxit=5000, forward_tol=1E-10, forward_maxit=100_000, **kwargs):
        """Evaluate steady state HetBlock using keyword args for all inputs. Analog to SimpleBlock.ss.
        ...
        if not utils.within_tolerance(D, Dold, forward_tol):
            for it in range(300):
                Dold = D
                kwargs['D'] = Dold
                sspol = self.policy_ss(kwargs, backward_tol, backward_maxit)
                D = self.dist_ss(Pi, sspol, grid, forward_tol, forward_maxit, Dold, pi_seed)
                if utils.within_tolerance(D, Dold, forward_tol):
                    break
            else:
                raise ValueError(f'No convergence after 300 iterations!')
    ...
    def td(self, ss, monotonic=False, returnindividual=False, **kwargs):
        ...
        if 'firm' in self.__repr__():
        #if self.__repr__() == '<HetBlock \'firm\'>' or self.__repr__() == '<HetBlock \'firm_ss\'>' or self.__repr__() == '<HetBlock \'firm_no_price_adj_costs\'>' or self.__repr__() == '<HetBlock \'firm_no_price_adj_costs_open\'>':
            #Pi_T = ss[self.exogenous + 'f'].T.copy()
            D = ss['Df']
        else:
            #Pi_T = ss[self.exogenous].T.copy()
            D = ss['D']
        ...
        for t in range(T - 1):
            ...
            for pol in self.policy:
                if monotonic:
                    ...
                    sspol_i[pol], sspol_pi[pol] = utils.interpolate_coord_robust(grid[pol],
        ...
        for i in shock_list:
            if 'firm' in self.__repr__():
                ...
                curlyYs[i], curlyDs[i] = self.backward_iteration_fakenews(i, output_list, ssin_dict, ssout_list,
                                                                          ss['Df'], Pi.T.copy(), sspol_i, sspol_pi,
                                                                          sspol_space, T, h,
                                                                          ss_for_hetinput)
            else:
                curlyYs[i], curlyDs[i] = self.backward_iteration_fakenews(i, output_list, ssin_dict, ssout_list,
                                                                          ss['D'], Pi.T.copy(), sspol_i, sspol_pi,
                                                                          sspol_space, T, h,
                                                                          ss_for_hetinput)
    ...
    def ajac(self, ss, T, shock_list, output_list=None, h=1E-6, Tpost=None, save=False, use_saved=False):
    ...
    def policy_ss(self, ssin, tol=1E-10, maxit=5000):
    ...
    def dist_ss(self, Pi, sspol, grid, tol=1E-12, maxit=100_000, D_seed=None, pi_seed=None):
    ...
    def jac_prelim(self, ss, save=False, use_saved=False):
        ...
        # ssin_dict = self.make_inputs(ss)
        # if self.__repr__() == '<HetBlock \'firm\'>':
        #     Pi = ss[self.exogenous + 'f']
        #     ssin_dict['e_grid'] = ss['ef_grid']
        #     ssin_dict['Pi_p'] = ss[self.exogenous + 'f']
        # else:
        #     Pi = ss[self.exogenous]
        #     ssin_dict['e_grid'] = ss['e_grid']
        #     ssin_dict['Pi_p'] = ss[self.exogenous]
        # grid = {k: ss[k + '_grid'] for k in self.policy}
        # ssout_list = self.back_step_fun(**ssin_dict)
        ...
        # if self.hetinput is not None:
        #     if self.__repr__() == '<HetBlock \'firm\'>':
        #         ss_for_hetinput['e_grid'] = ss['ef_grid']
        #     else:
        #         ss_for_hetinput['e_grid'] = ss['e_grid']
    ...
    def make_inputs(self, indict):
        ...
        if self.hetinput is not None:
            outputs_as_tuple = utils.make_tuple(
                self.hetinput(**{k: indict[k] for k in self.hetinput_inputs if k in indict}))
